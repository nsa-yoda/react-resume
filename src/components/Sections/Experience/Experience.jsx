        ...
        <div key={index} className={`job ${exp.meta.class} separator`}>
          <div className='row'>
            <div
              ...
              title={`${exp.dates.start} to ${exp.dates.end}`}
            >
              {formatDate(exp.dates.start)} {dateSeparator}{' '}
              {formatDate(exp.dates.end)}
            </div>
            <div className='col-4 float-right'>{exp.company.type}</div>
          </div>
          <div className='row'>
            <div className='title col-8'>
              <span className='blue-span'>{exp.title}</span> {separator}{' '}
              {exp.company.name}
            </div>
